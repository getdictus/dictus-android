...
static void insertCandidate(std::vector<Candidate>& results, const Candidate& c, int maxResults) {
    const auto duplicate = std::find_if(results.begin(), results.end(),
        [&c](const Candidate& existing) {
            return std::strcmp(existing.word, c.word) == 0;
        });
    if (duplicate != results.end()) {
        if (duplicate->score >= c.score) return;
        results.erase(duplicate);
    }
...
// Return how much of the typed word appears in order in a candidate. This
// distinguishes missing-letter typos ("bonjr" -> "bonjour") from unrelated
// words that happen to have the same weighted edit cost.
static float orderedInputCoverage(const uint16_t* input, int inputLen,
                                  const uint16_t* candidate, int candidateLen) {
    if (inputLen <= 0) return 0.0f;
    int previous[129] = {};
    int current[129] = {};
    const int boundedCandidateLen = std::min(candidateLen, 128);
    for (int inputIndex = 1; inputIndex <= inputLen; ++inputIndex) {
        for (int candidateIndex = 1; candidateIndex <= boundedCandidateLen; ++candidateIndex) {
            if (input[inputIndex - 1] == candidate[candidateIndex - 1]) {
                current[candidateIndex] = previous[candidateIndex - 1] + 1;
            } else {
                current[candidateIndex] = std::max(
                    previous[candidateIndex], current[candidateIndex - 1]);
            }
        }
        std::copy(current, current + boundedCandidateLen + 1, previous);
        std::fill(current, current + boundedCandidateLen + 1, 0);
    }
    return static_cast<float>(previous[boundedCandidateLen]) /
           static_cast<float>(inputLen);
}
...
static void searchRecursive(const Trie& trie,
    ...
    if (node.flags & FLAG_TERMINAL) {
        ...
        if (remainCost <= maxEditDist) {
            const float maxFreqF = static_cast<float>(trie.maxFreq());
            const float freqNorm = (maxFreqF > 0.0f)
                ? static_cast<float>(node.frequency) / maxFreqF
                : 0.0f;
            // Edit similarity is the base signal. Ordered input coverage gives
            // missing-letter corrections enough weight to survive a five-result
            // bound, while frequency only breaks otherwise similar candidates.
            const float distanceFactor =
                1.0f - remainCost / (maxEditDist + 1.0f);
            const float coverage =
                orderedInputCoverage(input, inputLen, wordBuf, wordLen);
            const float score = distanceFactor + coverage + 0.25f * freqNorm;
